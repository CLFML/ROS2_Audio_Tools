...
AudioCaptureNode::AudioCaptureNode()
  ...
  if (_channels < 1 || _channels > 8) {
    RCLCPP_ERROR_STREAM(this->get_logger(), "Invalid channel count: " << _channels);
  ...
  if (chunk_size < 512 || chunk_size > 4096) {
    RCLCPP_ERROR_STREAM(this->get_logger(), "Invalid chunk size: " << chunk_size);
  ...
  _pub_info = this->create_publisher<audio_tools::msg::AudioInfo>("audio_info", info_qos);
  
  auto pub_stamped = this->create_publisher<audio_tools::msg::AudioDataStamped>("audio_stamped", 10);
  ...
  _diagnosed_pub_stamped = std::make_shared<diagnostic_updater::DiagnosedPublisher<audio_tools::msg::AudioDataStamped>>(
...
void AudioCaptureNode::callback(uint8_t *stream, int len) {
  ...
  size_t buffer_size_total_nsec = (static_cast<size_t>(len) * 1e9) / (_sample_rate * _nbytes);
...
bool AudioCaptureNode::ConfigureSDLStream(int device, const std::string &format, int chunk_size, int channels) {
  ...
  for (int i = 0; i < nDevices; i++) {
    fprintf(stderr, "  - Capture device #%d: '%s'\n", i, SDL_GetAudioDeviceName(i, SDL_TRUE));
  }
  ...
  if (format_it == format_map.end()) {
    SDL_LogError(SDL_LOG_CATEGORY_APPLICATION, "Unknown audio format: %s", format.c_str());
    return false;
  }
  ...
  if (device >= 0 && device < nDevices) {
    fprintf(stderr, "%s: Attempting to open capture device #%d: '%s'...\n", 
            __func__, device, SDL_GetAudioDeviceName(device, SDL_TRUE));

    _dev_id_in = SDL_OpenAudioDevice(SDL_GetAudioDeviceName(device, SDL_TRUE), SDL_TRUE,
                                     &capture_spec_requested, &capture_spec_obtained, 0);
  } else {
    fprintf(stderr, "%s: Attempting to open default capture device...\n", __func__);
    _dev_id_in = SDL_OpenAudioDevice(nullptr, SDL_TRUE, &capture_spec_requested, &capture_spec_obtained, 0);
  }
