...
VADNode::VADNode() : Node("vad_node"), _current_vad_state(false) {
  ...
  RCLCPP_INFO(this->get_logger(), "  - Energy threshold: %.4f", _energy_threshold);
  ...
  _vad_pub = this->create_publisher<audio_tools::msg::VoiceActivity>("voice_activity", 10);
...
void VADNode::audioCallback(const audio_tools::msg::AudioDataStamped::SharedPtr msg) {
...
bool VADNode::convertToFloatSamples(const std::vector<uint8_t> &audio_data,
  ...
  if (it == _format_byte_size.end()) {
    RCLCPP_ERROR(this->get_logger(), "Unknown audio format: %s", sample_format.c_str());
    return false;
  }
  ...
  if (sample_format == "F32" || sample_format == "F32LE") {
    ...
    const int16_t *int_samples = reinterpret_cast<const int16_t*>(audio_data.data());
    ...
    const int8_t *int_samples = reinterpret_cast<const int8_t*>(audio_data.data());
    for (size_t i = 0; i < num_samples; i++) {
    ...
    for (size_t i = 0; i < num_samples; i++) {
      out_samples[i] = static_cast<float>(audio_data[i * bytes_per_sample]) / 255.0f;
    }
  }
